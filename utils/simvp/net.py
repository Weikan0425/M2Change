...
class SimVP_Model(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, act_inplace=True,out_dim=128, **kwargs):
        ...
        self.dec = Decoder(hid_S, out_dim, N_S, spatio_kernel_dec, act_inplace=act_inplace)
    ...
    def forward(self, x_raw, **kwargs):
        ...
        Y = self.dec(hid, skip)
        Y = Y.reshape(B, T, self.out_dim, H, W)
        return Y
...
if __name__ == '__main__':
    model = SimVP_Model((12, 2, 40, 40), model_type='tau', N_S=4, N_T=4, mlp_ratio=8., drop=0.0, drop_path=0.0).cuda()
    x = torch.randn(2, 12, 2, 40, 40).cuda()
    y = model(x)
    print(y.shape)
